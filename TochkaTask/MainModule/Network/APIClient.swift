...
class APIClient {
    ...
    static func fetch(completion: @escaping (Result<ViewData.ArticlesResponse, AFError>) -> Void) {
        ...
        AF.request(uri + NetworkUrl.topHeadlines.rawValue, parameters: params, encoding: URLEncoding(destination: .queryString), headers: headers).responseDecodable { (response: DataResponse<ViewData.ArticlesResponse, AFError>) in
